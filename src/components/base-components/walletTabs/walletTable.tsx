...
import RaiseADispute from '../../block-components/modal/raiseDispute'
...
export default function WalletTable({transactions}:IWalletTable) {
  ...
  return (
    <div className="text-orange-yellow">
      ...
      <div className="mt-8 flow-root min-h-[500px]">
              ...
              <tbody className="">
                {transactions?.map((transaction:any) => (
                  <tr key={transaction?._id} >
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatTime(transaction?.transactionDate)}</td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{transaction?.walletTransactionType}</td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-50">
                      <span
                        style={{
                          ...
                          color: transaction.status === 'Completed' ? '#33BB77' : transaction.status === "Ongoing" ? "#FFA500" : "#33BB77"
                        }}
                        className='px-4 py-2 bg-textPrimary rounded-lg text-sm font-bold'>
                        {"Completed"}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{transaction?.currency}  {formatMoney(Number(transaction?.transactionAmount))}</td>
