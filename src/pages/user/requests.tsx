...
import { filterRequestedOrders, formatOrderDate2, formatMoney, formatOrderDate } from '../../services/utils/helpersFunc'
...
function Requests() {
    ...
    const getOrders = async () => {
        ...
        if (res) {
            setRequests(filterRequestedOrders(res.data, 'Pending'))
        }
    }
    ...
    return (
        <div>
            ...
            <section className='mx-auto max-w-7xl items-center p-6'>
                <p className='my-5 font-bold md:text-xl'>Requests</p>

                <YebboBack
                    title='Requests'
                    screenDetails={["Grooves", "Requests"]}
                />

                {/* SECTION STARTS HERE */}

                <div className='flex flex-col md:flex-row justify-between  my-4 items-center'>
                    <p className='font-bold text-gray-700 md:mb-0 mb-4'>All Requests</p>
                </div>
                ...
                <div className='w-full mt-5 md:mt-10 flex flex-col md:flex-row justify-between '>
                    <div className='w-full md:w-1/3 bg-white md:p-10 p-2 h-full'>
                        ...
                        {requests?.length === 0 && 
                        <EmptyGrooveState message="There's currently no pending request" imageSrc=''/>
                        }
                        ...
                        {!requests && <LoadingState />}
                        {
                            requests && requests?.length > 0 && requests.map((request: any) => (
                                <button
                                    ...
                                    key={request} className='flex mb-5 hover:bg-[#EAEAEB] rounded-lg p-3  justify-start items-center cursor-pointer'>
                        ...
                        showDetails && (
                            <div className='w-full md:w-2/3 bg-white md:ml-5 md:p-10 p-2'>
                                ...
                                <div className='bg-[#FBFBFB] p-5 my-3 rounded-lg flex flex-col  justify-start'>
                                    ...
                                    <p className='font-bold text-charleston my-3'>TO BE IMPLEMENTED. NEEDED FROM BACKEND</p>
                                    <p
                                ...
                                <div className=' grid md:grid-cols-2 gap-3'>
                                    <div className='bg-[#FBFBFB] h-20 p-5 my-3 rounded-lg flex flex-col  justify-start'>
                                        <p className='text-xs mb-1  text-charleston'>{selectedGroove?.orderType === "perSession" ? "Session" : "Day"}</p>
                                        <p className='font-bold text-sm'>{selectedGroove?.orderType === "perSession" ? `${selectedGroove?.sessionTime}` : selectedGroove?.date && formatOrderDate(selectedGroove?.date)}</p>
                                    </div>
